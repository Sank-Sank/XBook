...
//        if (!isBookOpen) return;
    ...
    public void setBgColor(int theme) {
        if (isNightMode && theme == ReadSettingManager.NIGHT_MODE) {
            ...
            mPageBg = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.black);
        } else if (isNightMode) {
            mBgTheme = theme;
            ...
            switch (theme) {
                case ReadSettingManager.READ_BG_DEFAULT:
                    mTextColor = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_2c);
                    mPageBg = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_cec29c);
                    break;
                case ReadSettingManager.READ_BG_1:
                    mTextColor = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_2f332d);
                    mPageBg = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_ccebcc);
                    break;
                case ReadSettingManager.READ_BG_2:
                    mTextColor = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_92918c);
                    mPageBg = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_aaa);
                    break;
                case ReadSettingManager.READ_BG_3:
                    mTextColor = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_383429);
                    mPageBg = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_d1cec5);
                    break;
                case ReadSettingManager.READ_BG_4:
                    mTextColor = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_627176);
                    mPageBg = ContextCompat.getColor(MyApp.Companion.getInstance(), R.color.color_001c27);
                    break;
            }
        }
...
//        isBookOpen = false;
    ...
    /***********************************default method***********************************************/
    //获取Page的方法
    List<TxtPage> loadPages(String title, String content) {
        //生成的页面
        List<TxtPage> pages = new ArrayList<>();
        List<String> lines = new ArrayList<>();                     //一页的文字
        TxtPage page = new TxtPage();                               //单页
        int rHeight = mVisibleHeight;                               //界面的高
        float fontWidth = mTitlePaint.measureText("哈");
        int wordCount = (int) (mVisibleWidth / fontWidth);          //一行所占的字数
        String subStr = "";
    ...
    void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            if (mStatus != STATUS_FINISH) {
                ...
                canvas.drawText(Objects.requireNonNull(mCurPage.getTitle()), mMarginWidth, tipTop, mTipPaint);
            }
